...
export default function Shop() {
  ...
  const categories = [
    'Classics', 'Fiction', 'Historical Fiction', 'School', 'Literature', 
    'Young Adult', 'Historical', 'Novels', 'Read For School', 'High School'
  ];
  ...
  async function getAllProducts() {
    await axios.get("http://localhost:3001/books/getAllBooks", {
      headers: {
        Authorization: token,
      },
    })
  ...
  async function getBooksByCategory(category) {
    await axios.get(
      `http://localhost:3001/books/getBooksByCat/${category}`,
      {
        headers: {
          Authorization: token,
        },
